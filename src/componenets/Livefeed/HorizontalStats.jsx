...
export const HorizontalStats = ({ posts, user }) => {
  ...
  const handleScroll = (direction) => {
    ...
    const scrollAmount = cardWidth + (window.innerWidth < 640 ? 12 : 16);
  ...
  return (
    <div className="relative w-full mx-auto my-4 sm:my-6 px-4 sm:px-6 custom:px-8">
      ...
      <div
        ref={statsRef}
        className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 custom:flex custom:flex-nowrap custom:gap-4 custom:overflow-x-auto custom:scroll-smooth custom:py-2 custom:no-scrollbar"
        style={{
          scrollSnapType: window.innerWidth >= 1423 ? 'x mandatory' : 'none',
        }}
      >
